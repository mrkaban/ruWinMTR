...
BEGIN_MESSAGE_MAP(WinMTRDialog, CDialog)
	...
	ON_NOTIFY(LVN_ITEMCHANGED, IDC_LIST_MTR, &WinMTRDialog::OnLvnItemchangedListMtr)
END_MESSAGE_MAP()
...
BOOL WinMTRDialog::OnInitDialog()
{
	...
	#ifndef  _WIN64
	char caption[] = {"ruWinMTR v0.92 русская версия"};
	#else
	char caption[] = {"ruWinMTR v0.92 русская версия"};
	#endif
	...
	if(!statusBar.Create( this ))
		AfxMessageBox("Ошибка при создании строки состояния");
	...
	{ // Add appnor URL
		...
		if (!m_pWndButton->Create(_T("континентсвободы.рф"), WS_CHILD|WS_VISIBLE|WS_TABSTOP, CRect(0,0,0,0), &statusBar, 1234)) {
			TRACE(_T("Не удалось создать кнопки управления.\n"));
			return FALSE;
		}

		m_pWndButton->SetURL("http://xn--90abhbolvbbfgb9aje4m.xn--p1ai/");
			
		if(!statusBar.AddPane(1234,1)) {
			AfxMessageBox(_T("Индекс панели вне диапазона \ ни панели с таким же ID уже существует в строке состояния"), MB_ICONERROR);
			return FALSE;
		}
	...
	if(state == TRACING) {
		...
		if(pos!=NULL) {
			...
			if(wmtrnet->GetAddr(nItem)==0) {
				...
				strcpy(wmtrprop.comment , "Хост живой.");
...
{	
	...
	strcpy(f_buf,  "|------------------------------------------------------------------------------------------|\r\n");
	sprintf(t_buf, "|                                    ruWinMTR Статистика                                   |\r\n");
	strcat(f_buf, t_buf);
	sprintf(t_buf, "|                       Хост             -   %%  | Отпр |Получ | Лучш |Средн | Худш | Посл |\r\n" ); 
	strcat(f_buf, t_buf);
	sprintf(t_buf, "|------------------------------------------------|------|------|------|------|------|------|\r\n" ); 
	strcat(f_buf, t_buf);
	...
	for(int i=0;i <nh ; i++) {
		...
		if(strcmp(buf,"")==0) strcpy(buf,"Нет ответа от хоста");
...
void WinMTRDialog::OnCHTC() 
{
	...
	strcpy(f_buf, "<html><head><title>ruWinMTR Статистика</title></head><body bgcolor=\"white\">\r\n");
	sprintf(t_buf, "<center><h2>ruWinMTR Статистика</h2></center>\r\n");
	strcat(f_buf, t_buf);
	
	sprintf(t_buf, "<p align=\"center\"> <table border=\"1\" align=\"center\">\r\n" ); 
	strcat(f_buf, t_buf);
	
	sprintf(t_buf, "<tr><td>Хост</td> <td>%%</td> <td>Отпр</td> <td>Получ</td> <td>Лучш</td> <td>Средн</td> <td>Худш</td> <td>Посл</td></tr>\r\n" ); 
	strcat(f_buf, t_buf);
...
//*****************************************************************************
// WinMTRDialog::OnEXPT
// Хост             -   %%  | Отпр |Получ | Лучш |Средн | Худш | Посл
// 
//*****************************************************************************
...
void PingThread(void *p)
{
	...
	if(lhost == NULL) {
      AfxMessageBox("Невозможно получить локальный IP-адрес.");
...
void WinMTRDialog::Transit(STATES new_state)
{
	switch(new_state) {
		case IDLE:
			switch (state) {
				...
				break;
				default:
					TRACE_MSG("Полученное состояние IDLE после " << state);
					return;
			}
			state = IDLE;
		break;
		case TRACING:
			switch (state) {
				...
				break;
				default:
					TRACE_MSG("Полученное состояние TRACING после " << state);
					return;
			}
			state = TRACING;
		break;
		case STOPPING:
			switch (state) {
				...
				break;
				default:
					TRACE_MSG("Полученное состояние STOPPING после " << state);
					return;
			}
			state = STOPPING;
		break;
		case EXIT:
			switch (state) {
				...
				break;
				default:
					TRACE_MSG("Полученное состояние EXIT после " << state);
					return;
			}
			state = EXIT;
		break;
		default:
			TRACE_MSG("Полученное состояние " << state);
	}
	...
	switch(transition) {
		case IDLE_TO_TRACING:
			...
			m_buttonStart.SetWindowText("Остановить");
			m_comboHost.EnableWindow(FALSE);
			m_buttonOptions.EnableWindow(FALSE);
			statusBar.SetPaneText(0, "Двойной щелчок по имени хоста для получения дополнительной информации.");
		...
		case STOPPING_TO_IDLE:
			m_buttonStart.EnableWindow(TRUE);
			statusBar.SetPaneText(0, CString((LPCSTR)IDS_STRING_SB_NAME) );
			m_buttonStart.SetWindowText("Начать");
		...
		case TRACING_TO_STOPPING:
			...
			m_buttonOptions.EnableWindow(FALSE);
			wmtrnet->StopTrace();
			statusBar.SetPaneText(0, "Ожидание последних пакетов для того, чтобы остановить трассировку...");
			DisplayRedraw();
		break;
		case IDLE_TO_EXIT:
			m_buttonStart.EnableWindow(FALSE);
		...
		break;
		default:
			TRACE_MSG("Неизвестный переход " << transition);
	}
}
...
void WinMTRDialog::OnLvnItemchangedListMtr(NMHDR *pNMHDR, LRESULT *pResult)
{
